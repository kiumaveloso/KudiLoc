...
export async function getATMsByBank(
  bankName: string,
  page = 1,
  pageSize = 20,
): Promise<PagedResult<ATMDto>> {
  return apiFetch<PagedResult<ATMDto>>(
    `/atm/bank/${encodeURIComponent(bankName)}?page=${page}&pageSize=${pageSize}`,
  );
}

/** Get ATMs by province. */
export async function getATMsByProvince(
  province: string,
  page = 1,
  pageSize = 20,
): Promise<PagedResult<ATMDto>> {
  return apiFetch<PagedResult<ATMDto>>(
    `/atm/province/${encodeURIComponent(province)}?page=${page}&pageSize=${pageSize}`,
  );
}
...
/** Post a comment on an ATM (requires auth). */
export async function addComment(
  ...
  return apiFetch<CommentDto>('/comment', {
    method: 'POST',
    body: JSON.stringify({ atmId, userId, userName, text }),
  });
}
